...
import { ExternalLink, Users, ShieldAlert, Landmark } from "lucide-react";

export function SkillUpEdis({ data }: { data?: ProjectMetrics }) {
  if (!data) return <EmptyState title="SKILLUP_EDIS_C2K" />;

  const screenshot = data.screenshotPath;

  return (
    <TerminalWrapper title="enterprise_project.01_Ignite EdIs C2k" className="col-span-full">
      <div className="flex flex-col h-full space-y-4 justify-between">
        <div className="space-y-3">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[10px] text-slate-500 font-mono">COMPLIANCE_LMS_PORTAL</span>
        ...
        <div className="grid grid-cols-2 gap-4 py-3 border-y border-slate-900 my-2">
          <div className="flex items-center space-x-2">
            <Users className="w-4 h-4 text-emerald-400" />
            <div className="flex flex-col">
              <span className="text-[9px] text-slate-500 font-mono">CAPACITY</span>
              <span className="text-xs text-slate-200 font-mono font-semibold">
                {data.performance?.users || "N/A"}
              </span>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] text-slate-500 font-mono">PEAK_CONCURRENCY</span>
              <span className="text-xs text-slate-200 font-mono font-semibold">
                {data.performance?.concurrency || "N/A"}
              </span>
            </div>
          </div>
        </div>
        ...
        {data.customerDetails && (
          <div className="flex items-center space-x-2 pb-2 text-xs text-slate-400 font-mono">
